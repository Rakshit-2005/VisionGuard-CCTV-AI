...
// import { companyApi, PendingCompany } from '@/lib/CompanyApi';
...
//   const [pendingRequests, setPendingRequests] = useState<PendingCompany[]>([]);
...
export default function PlatformDashboard() {
  ...
  const activeCompanies = mockCompanies.filter(c => c.status === 'active').length;
  const pendingCompaniesCount = mockCompanies.filter(c => c.status === 'pending').length;
  const totalEmployees = mockCompanies.reduce(
    (acc, c) => acc + c.employeeCount,
    0
  );
  const todayViolations = mockViolations.filter(v => {
    const today = new Date().toDateString();
  ...
  useEffect(() => {
    const fetchPendingRequests = async () => {
      try {
        ...
        const data = await apiRequest('/company/requests');
        setPendingRequests(data);
      } catch (error) {
        console.error('Failed to fetch pending company requests', error);
      } finally {
  ...
  const handleApprove = async (id: string) => {
    try {
      await apiRequest(`/company/approve/${id}`, { method: 'POST' });
  ...
  const handleReject = async (id: string) => {
    try {
      await apiRequest(`/company/reject/${id}`, { method: 'POST' });
      setPendingRequests(prev => prev.filter(c => c._id !== id));
    } catch (error) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="chart-container">
          <h3 className="font-semibold mb-2">Compliance Trend</h3>
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={complianceTrendData}>
        ...
        <div className="chart-container">
          <h3 className="font-semibold mb-2">Violations by Company</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={violationsByCompany} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis type="number" />
              ...
              <Tooltip />
              <Bar dataKey="violations" fill="hsl(var(--warning))" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      {/* =========================
          Pending Company Approvals (REAL API)
      ========================= */}
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {company.admin_name} · {company.industry_type}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="pending">Pending</Badge>
                    <Button size="sm" onClick={() => handleApprove(company._id)}>
                      Approve
                    </Button>
                    <Button
                      size="sm"
      ...
      {/* =========================
          Active Companies (mock – unchanged)
      ========================= */}
      <div className="glass-panel p-6">
        <div className="flex items-center justify-between mb-4">
